...
class UI{
    displayProducts(products){
        ...
        products.forEach(product=>{
            ...
            productive.innerHTML = `<div class = "product-cart">
                                     <img src="${product.image}"alt="product">
                                     <spam class= "add-to-cart" data-id="${product.id}">
    ...
    getButtons(){
        ...
        btns.forEach((btn)=>{
            ...
            btn.addEventListener("click",(e)=>{
                ...
                let caerItem = {...Storage.getStorageProducts(id),'amount':1}
                Cart.push(cartItem)
            ...
            cartContent.addEventListener("click",(event)=>{
                if(event.target.classList.contains("cart-product-remove")){
                    ...
                    if(item.amount>1){
                     ...
                     Storage.saveCart(cart)
                     this.setCartValues(cart)
                     event.target.previusElementSibling.innerHTML = item.amount
                    }
                    else{
                         this.removeItem(id)
                         let div= event.target.parentElement.parentElement.parentElement.parentElement
